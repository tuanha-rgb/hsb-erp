...
import { Clock, FileText, Users, TrendingUp, BookOpen, Calendar, 
  CheckSquare, AlertCircle, Award, MessageSquare, Bell, ChevronRight, 
  BarChart3, Home, User, BookText, Globe, Lock, ChevronLeft, Mail, 
  Phone, Building2, Edit, GraduationCap  } from 'lucide-react';
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-right">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{lecturerInfo.rating}</p>
                  <span className="text-xs text-green-600 font-semibold">{lecturerInfo.ratingLevel}</span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {teachingPerformance.map((sem, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h3 className="font-semibold text-gray-900">{sem.semester}</h3>
                        <p className="text-xs text-gray-600 mt-1">Avg. Rating</p>
                      </div>
                      <span className={`px-2 py-1 text-xs font-semibold rounded ${sem.levelColor}`}>
                        {sem.level}
                      </span>
                    </div>
                    <p className="text-2xl font-bold text-gray-900 mb-2">{sem.gpa}</p>
                    <div className="text-xs text-gray-600 space-y-1">
                      <p>Courses: {sem.courses}</p>
                      <p>Students: {sem.students}</p>
                    </div>
                  </div>
                ))}
              </div>
